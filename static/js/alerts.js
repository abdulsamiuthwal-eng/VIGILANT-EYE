...
const AlertSystem = (() => {
  ...
  function init() {
    ...
    socket.on("stats_update", (data) => Charts && Charts.updateStats(data));
  }
  ...
  function viewSnapshot(path, activityType, cameraName, timestamp) {
    ...
    img.onerror = () => { img.src = ""; img.alt = "Snapshot unavailable"; };
    info.innerHTML = `
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("testAlertBtn")?.addEventListener("click", () => {
    fetch("/api/alerts/test", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({camera_id:1}) })
      .then(r => r.json()).then(d => console.log("Test alert:", d));
  });
